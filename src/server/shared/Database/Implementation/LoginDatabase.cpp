...
bool LoginDatabaseConnection::Open()
{
    ...
    */
    ...
    PrepareStatement(LOGIN_GET_SESSIONKEY, "SELECT sessionkey,id,gmlevel  FROM account WHERE username = ?");
    ...
    PrepareStatement(LOGIN_GET_LOGONCHALLENGE, "SELECT sha_pass_hash, id, locked, last_ip, gmlevel, v, s FROM account WHERE username = ?");
